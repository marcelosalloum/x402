...
const STELLAR_TESTNETS = new Set<Network>(["stellar-testnet"]);
...
export function getPreferredNetworks(testnet: boolean): Network[] {
  if (testnet) {
    return ["base-sepolia", "solana-devnet", "stellar-testnet"];
  }
  return ["base", "solana", "stellar"];
}
 ...
 */
export function isStellarNetwork(network: string): network is Network {
  return SupportedStellarNetworks.includes(network as Network);
}
...
export function isTestnetNetwork(network: Network): boolean {
  return EVM_TESTNETS.has(network) || SVM_TESTNETS.has(network) || STELLAR_TESTNETS.has(network);
}
